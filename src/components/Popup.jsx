      ...
      <section className="fixed w-full h-full grid place-items-center bg-myGreen z-50 left-0">
        <div className="popup w-11/12 md:w-9/12 h-[85%] md:h-[80%] xl:h-[75%] bg-myYellow flex flex-col rounded-xl md:items-center shadow-myShadow">
          ...
          <div className="md:w-full">
            <h2 className="pt-10 lg:py-3 lg:hidden  xl:py-7 text-3xl xl:text-5xl font-dancing font-bold">{allProjects[project].name}</h2>
            ...
            {allProjects[project].tech.map( (tech, index) => (
              ...
              'font-extrabold text-white leading-4 md:leading-4 md:p-2 border-2 border-myBlue bg-myPurple rounded p-1 text-sm' : 'noDisplay'}>
                {tech}
              </li>
              ))}
            </ul>
            <img src={allProjects[project].image} alt="algo" className="w-10/12 mx-auto lg:hidden mb-4 md:mb-1 lg:mt-2 border-2 
          ...
          <div className="w-11/12 lg:w-[95%] flex flex-col lg:flex-row justify-around lg:gap-10 items-center lg:items-start  mx-auto md:h-5/6 lg:h-full">
            <img src={allProjects[project].image} alt="algo" className="w-7/12 hidden lg:inline lg:max-w-[380px] xl:max-w-[600px] mx-auto
            mb-4 lg:mt-2 border-2 border-myGray rounded shadow-myShadow" />
            <p className="lg:w-4/12 xl:w-5/12 text-left md:text-lg lg:min-w-[300px]">{allProjects[project].description}</p>
            <div className="absolute bottom-3 md:relative lg:absolute lg:bottom-8 md:bottom-auto flex justify-between xl:justify-end
              w-8/12 xl:w-11/12 mx-auto min-w-[250px] xl:gap-6">

              <a href={allProjects[project].live} target="_blank" className="bg-myYellow p-2 w-3/12 max-w-[200px] font-bold active:bg-myGray
              ...
              rel="noreferrer">See live<GrDeploy className="w-4 inline group-hover:translate-x-2 group-hover:scale-125 translate-x-2 transition-all" /></a>

              <a href={allProjects[project].source} target="_blank" className="bg-myYellow p-1 w-4/12 max-w-[200px] font-bold active:bg-myGray
              active:text-white hover:bg-myHoverButton shadow-myShadow border border-myGray hover:translate-x-2 hover:translate-y-2
              hover:text-myBlue flex justify-center items-center group hover:shadow-none transition-all min-w-[120px] xl:w-2/12" 
              rel="noreferrer">See Source<FiGithub className="w-4 inline group-hover:translate-x-2 group-hover:scale-125 translate-x-2 transition" /></a>
            </div>
          </div>
        </div>
      </section>
