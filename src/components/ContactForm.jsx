...
function ContactForm() {
  ...
  return (
    <section className="grid  h-screen leftSide md:ml-[43%] lg:ml-[40%] xl:ml-[45%]" id="contactForm">
      ...
      <form ref={refContact} className="w-10/12 max-w-md mx-auto" action="https://formspree.io/f/myyanyge" method="POST" target="_blank">
        ...
        <div className={`relative z-0 w-full mb-5 group ${inViewContactName ? 'moveLeft' : ''}`} ref={refContactName}>
          <input type="text" name="floating_name" className="block py-2.5 px-0 w-full text-lg text-myGray
          ...
          transform -translate-y-6 scale-75 top-3 left-0 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto
        ...
        <div className={`relative z-0 w-full mb-5 group ${inViewContactEmail ? 'moveRight' : ''}`} ref={refContactEmail}>
          <input type="email" name="floating_email" className="block py-2.5 px-0 w-full text-lg text-myGray
          bg-transparent border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-myPurple peer" placeholder=" " required />
          ...
          <textarea rows="4" ref={refContactMessage} className={`block p-2.5 w-full text-lg text-myGray bg-myYellow rounded-lg border
        ...
        <button ref={refContactMessage} className={`bg-myYellow p-2 w-2/4 font-bold active:bg-myGray active:text-white shadow-myShadow 
